...
// ─── Table Picker Pop-up Modal ──────────────────────────────────────────────
function TablePickerModal({ tables: initialTables = [], selectedTableNumber, onSelectTable, onClose }) {
    ...
    if (initialTables && initialTables.length > 0) {
      ...
      (async () => {
        try {
          ...
          const res = await api.get('/reception/tables').catch(() => api.get('/pos/menu'));
          const tbls = Array.isArray(res?.data?.data) ? res.data.data : (res?.data?.data?.tables || []);
  ...
  const areas = ['All', ...new Set(tablesList.map(t => t.locationArea).filter(Boolean))];

  const filteredTables = tablesList.filter(t => {
    const matchesArea = selectedArea === 'All' || t.locationArea === selectedArea;
    const matchesSearch = !search || (t.tableNumber && t.tableNumber.toLowerCase().includes(search.toLowerCase()));
    return matchesArea && matchesSearch;
  });

  const availableCount = tablesList.filter(t => t.status === 'Available' || !t.status).length;
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 250 }} onClick={onClose}>
      <div
        className="modal"
        style={{ maxWidth: 540, maxHeight: '82vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <h2 className="modal-title" style={{ fontSize: 15 }}>Select Dining Table</h2>
              <div style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>
                {availableCount} Available · {occupiedCount} Occupied · Total {tablesList.length} Tables
              </div>
            </div>
          </div>
          <button className="btn-icon btn-sm" onClick={onClose}><X size={16} /></button>
        ...
        {/* Filter bar */}
        <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-primary)', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <div className="search-box" style={{ flex: 1, minWidth: 150 }}>
            <Search size={14} />
            <input
              type="text"
              className="search-input"
              style={{ fontSize: 12.5, padding: '7px 10px 7px 32px' }}
            ...
            {areas.map(area => (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${selectedArea === area ? 'var(--accent)' : 'var(--border)'}`,
                  background: selectedArea === area ? 'var(--accent)' : 'var(--bg-card)',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
          {loadingTables ? (
            <div style={{ textAlign: 'center', padding: '30px 10px', color: 'var(--text-muted)', fontSize: 13 }}>
              <RefreshCw size={18} className="spin" style={{ display: 'inline', marginRight: 6 }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10 }}>
              {filteredTables.map(t => {
                ...
                return (
                  <button
                    key={t._id || t.tableNumber}
                    type="button"
                    onClick={() => {
                      onSelectTable(t.tableNumber, t.capacity || 4);
                      onClose();
                    }}
                    style={{
                      padding: '12px 10px',
                      borderRadius: 'var(--radius-sm)',
                      border: `2px solid ${isSelected ? 'var(--accent)' : isOccupied ? 'rgba(220,38,38,0.3)' : 'var(--border)'}`,
                      background: isSelected ? 'var(--accent-soft)' : isOccupied ? 'var(--red-soft)' : 'var(--bg-card)',
                      color: isSelected ? 'var(--accent)' : isOccupied ? 'var(--red)' : 'var(--text-primary)',
                      cursor: 'pointer',
                      textAlign: 'left',
                      display: 'flex',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 14, fontWeight: 800 }}>Table {t.tableNumber}</span>
                      <span style={{
                        fontSize: 10,
                        fontWeight: 700,
                        padding: '2px 5px',
                        borderRadius: 4,
                        background: isOccupied ? 'rgba(220,38,38,0.18)' : isReserved ? 'rgba(234,179,8,0.18)' : 'rgba(34,197,94,0.18)',
                        color: isOccupied ? 'var(--red)' : isReserved ? '#B45309' : 'var(--green)'
                      }}>
...
// ─── Checkout Modal ─────────────────────────────────────────────────────────
function CheckoutModal({ cart, pricing, orderType = 'dineIn', onClose, onSuccess, restaurants, tables = [] }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (customerEmail) {
      ...
      const sanitizedPhone = customer.phone.replace(/[^0-9]/g, '') || Date.now();
    ...
    try {
      ...
      const res = await api.post('/pos/orders', {
        ...
        customer: {
          ...
          city: 'Lahore',
          area: '',
          address: currentOrderType === 'dineIn' ? (tableNumber ? `Table ${tableNumber}` : 'Walk-In') : (customer.address || (currentOrderType === 'pickup' ? 'Takeaway' : 'Lahore'))
        },
        paymentMethod,
        tableNumber: currentOrderType === 'dineIn' ? tableNumber : '',
        guestCount: currentOrderType === 'dineIn' ? guestCount : 1,
        idempotencyKey
      }, {
    ...
    <>
      ...
      {showTablePicker && (
        <TablePickerModal
          ...
          onSelectTable={(tblNum, cap) => {
            setTableNumber(tblNum);
            if (cap) setGuestCount(cap);
          }}
...
export default function POSPage() {
  ...
  return (
    <div style={{ margin: '-24px', height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* ── POS Top Bar ── */}
      <div className="pos-topbar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '0 20px', height: 52, background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
