    ...
    try {
      ...
      socket.on('order:created', (order) => {
        const orderNotif = {
          ...
          message: `${order.customer?.name || 'Guest'} created ${order.orderType || 'dine-in'} order (${order.items?.length || 1} items).`,
  ...
  return (
    <div style={{ position: 'relative' }} ref={dropdownRef}>
      ...
      <button
        ...
        title="Live Reception Notifications"
        onClick={() => {
          setOpen(v => !v);
            ...
            {loading && notifications.length === 0 ? (
              ...
              <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-muted)' }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', marginBottom: 2 }}>
                  No Front Desk Alerts
                </div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  All orders and table actions are up to date.
                </div>
              </div>
            ) : (
